...
def get_device(device: Optional[str | torch.device] = None) -> torch.device:
    ...
    if 'cuda' in device:
        ...
        print(f"Before setting CUDA_VISIBLE_DEVICES, current device is: {torch.cuda.current_device()}, visible are {curr_viz}")
        os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_num)
        curr_viz = os.environ.get("CUDA_VISIBLE_DEVICES", "")
        print(f"After setting CUDA_VISIBLE_DEVICES, current device is: {torch.cuda.current_device()}, visible are {curr_viz}")
