...
IMDB_X_TXT_FIRST_ROW = ['Target Feature: Genre\nFeature Value: Drama',
...
IMDB_X_TXT_LAST_ROW = ['Target Feature: Genre\nFeature Value: Drama',
 'Target Feature: Genre\nFeature Value: Not Drama',
...
def test_tabstar_preprocessing_imdb():
    ...
    # First example
    assert y[0] == 1
    assert set(x_num[0].tolist()) == set(IMDB_X_NUM_FIRST_ROW)
    assert set(x_txt[0].tolist()) == set(IMDB_X_TXT_FIRST_ROW)

    # Last example
    assert y[-1] == 0
    assert set(x_num[-1].tolist()) == set(IMDB_X_NUM_LAST_ROW)
    assert set(x_txt[-1].tolist()) == set(IMDB_X_TXT_LAST_ROW)
    _rm_temp_dir()
